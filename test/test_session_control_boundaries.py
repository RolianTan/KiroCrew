...
class TestMirroredSessionsAreOutOfBounds:
    def _pair(self, tmp_path):
        ...
        # Sending is restricted to sessions the caller created, so the pair these
        # boundary tests share has to establish that first; otherwise every send
        # is refused as target_not_owned before the boundary under test is reached.
        target.created_by = caller.key
...
class TestCrewModeTargetsAreOutOfBounds:
    ...
    def _pair(self, tmp_path):
        ...
        caller = _slot(state, "caller")
        target = _slot(state, "peer")
        # Sending only reaches sessions the caller created (see the ownership gate
        # in authorize_target), so establish that before testing this boundary.
        target.created_by = caller.key
        return state, caller, target
    ...
    def test_the_refusal_precedes_delivery(self, tmp_path):
        """No turn may start before the refusal -- that is the whole defect."""
        state, caller, target = self._pair(tmp_path)
        target.mode = "crew"
        with patch(
            "kiro_crew.dashboard.turn_dispatch.spawn_guarded_turn"
        ) as spawn:
...
class TestTheRoutesRequireTheInternalSecret:
    @pytest.mark.parametrize(
        "handler,path",
        [
            (handlers_sc.api_session_control_send, "/api/session-control/send"),
    ...
    def test_the_refusal_names_the_reason(self):
        resp = asyncio.run(
            handlers_sc.api_session_control_send(
                _request("/api/session-control/send", internal=False)
            )
        )
        assert b"internal_secret_required" in resp.body
...
class TestRouteBodyValidation:
    ...
    def test_a_non_object_body_is_400(self, tmp_path):
        state = _make_state(tmp_path)
        resp = asyncio.run(
            handlers_sc.api_session_control_send(
                _request(
        ...
        assert resp.status == 400
        assert b"invalid_body" in resp.body
