...
class TestRetirementCoverage:
    ...
    @pytest.mark.asyncio
    async def test_a_cold_start_in_flight_makes_the_sweep_incomplete(self) -> None:
        """The barrier: every cold-start permit must be free before scanning.

        Checking for in-flight starts AFTER the scan cannot close the window -- a
        start that registers DURING the scan is missed by the session sweep and by
        the check. Holding all permits up front means nothing can register while we
        look.
        """
    ...
    @pytest.mark.asyncio
    async def test_the_barrier_never_waits_on_another_cold_start(self) -> None:
        """Permits are taken only while free, so a turn is never blocked behind one.

        Waiting would also mean cancelling an ``acquire()`` on timeout, which can
        leak a permit on some Python versions and permanently shrink cold-start
        concurrency.
        """
        ...
        await smap._start_sem.acquire()
        try:
            done = await asyncio.wait_for(smap.retire_kiro_identity_sessions(), timeout=1.0)
    ...
    @pytest.mark.asyncio
    async def test_one_held_permit_is_observable(self) -> None:
        """Pins the private-``_value`` relationship the in-flight check relies on.

        ``locked()`` is true only when ALL permits are taken, so it would miss one
        start among four. If the standard library changes how the count is exposed,
        this fails rather than the guard silently reading "nothing in flight".
        """

        smap = self._manager()
        assert smap._cold_starts_in_flight() is False
        await smap._start_sem.acquire()
        try:
            assert smap._cold_starts_in_flight() is True
            assert smap._start_sem.locked() is False, (
                "one of four permits held must NOT read as fully locked -- "
                "that is why locked() cannot be used here"
            )
        finally:
            smap._start_sem.release()
        assert smap._cold_starts_in_flight() is False
