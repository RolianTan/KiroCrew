...
Three things are checked, each unrecoverable once clients cache the feed:

* the COFF machine matches the architecture the feed will claim,
...
def _read_pe_security_blob(installer: Path) -> tuple[int, bytes]:
    ...
    raise VerificationError(
        f"{installer} has a certificate table with no PKCS#7 signed-data entry"
    )
...
def verify(installer: Path, expect_machine: int, expect_subject_cn: str) -> list[str]:
    ...
    if machine != expect_machine:
        raise VerificationError(
            f"{installer} is built for machine {machine:#06x}, but the feed would "
            f"advertise {expect_machine:#06x}. Clients would download a binary for "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--expect-machine",
        required=True,
        help="COFF machine the feed will advertise, e.g. 0x8664 for x64",
    )
    parser.add_argument("--expect-subject-cn", required=True)
    args = parser.parse_args(argv)

    try:
        expect_machine = int(args.expect_machine, 16)
    except ValueError:
        print(f"::error::--expect-machine {args.expect_machine!r} is not hexadecimal")
        return 2

    try:
        findings = verify(args.installer, expect_machine, args.expect_subject_cn)
    except VerificationError as exc:
        print(f"::error::{exc}")
        return 1
