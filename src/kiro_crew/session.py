...
class SessionManager:
    ...
    async def retire_kiro_identity_sessions(self) -> tuple[list[str], bool]:
        ...
        * companion runtimes keyed by a retired session, via
        ...
        # was not there when we looked) and by both in-flight checks (it is no
        # longer starting), so the baseline would advance over a provider holding
        # the previous account. Taking every cold-start permit up front means no
        # start can be in progress or begin while we look, so anything that
        # finished is already registered and anything that has not is blocked.
        #
        # Acquired WITHOUT waiting: permits are only taken while `_value` says they
        # are free, so this never blocks a turn behind someone else's cold start
        # and never cancels an `acquire()` (a cancelled acquire can leak a permit on
        # some Python versions, permanently shrinking cold-start concurrency).
        # Failing to take them all means a start IS in flight, which is reported as
        # incomplete and re-swept next turn.
        held = 0
        while held < _MAX_CONCURRENT_COLD_STARTS and self._start_sem._value > 0:
            await self._start_sem.acquire()
            held += 1
        if held < _MAX_CONCURRENT_COLD_STARTS:
            skipped = True
        try:
            async with self._lock:
    ...
    def _cold_starts_in_flight(self) -> bool:
        """Whether any cold start currently holds a ``_start_sem`` permit.

        ``asyncio.Semaphore`` exposes no public count, and ``locked()`` is true only
        when ALL permits are taken -- so it would miss one start among four. The
        held count is derived from the private ``_value`` against the named cap;
        ``test_one_held_permit_is_observable`` pins that relationship so a change in
        the standard library breaks a test rather than silently disabling this
        guard.
        """

        return _MAX_CONCURRENT_COLD_STARTS - self._start_sem._value > 0
