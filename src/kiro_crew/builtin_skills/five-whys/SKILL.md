...
```
python3 scripts/five_whys.py ask     <log> --parent ID|root --stage S --q "..." [--origin proposed|user]   # prints new id
python3 scripts/five_whys.py answer  <log> --id ID --a "..." [--source ai|user|<cite>]
python3 scripts/five_whys.py focus    <log> --id ID
python3 scripts/five_whys.py done     <log> --id ID [--kind root|takeaway] [--note "..."]
python3 scripts/five_whys.py discuss  <log> --anchor ID --text "..."
python3 scripts/five_whys.py prune    <log> --id ID [--reason "..."]
python3 scripts/five_whys.py event    <log> --json '{"type":"search",...}'   # plugin custom types
python3 scripts/five_whys.py view     <log>      # current path root->focus + open-branch count (show this each turn)
python3 scripts/five_whys.py tree     <log> | frontier <log> | report <log> [--title "..."]
...
2. **Give the first answer / orientation, then propose the first menu** of 2-4
   branch questions. Append the `answer`; append an `ask` for each question the
   user then chooses. Stop.
