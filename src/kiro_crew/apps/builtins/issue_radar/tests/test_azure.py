...
class TestPermissionsFailClosed(unittest.TestCase):
    """An unreadable team roster must not grant write access.

    ``routes._repo_can_write`` consults this object before admitting a mutation
    and caches the answer, so "we could not tell" has to land on the same side as
    "not allowed". Azure re-checks each write and answers 403, but that is a
    second gate, not a reason for this one to be permissive.
    """

    OWNER = "contoso/Widgets"

    def _perms(self, *, roster, login="ada"):
        """Permissions with the roster lookup stubbed to ``roster``.

        ``roster`` is either a list of rows or an exception instance to raise.
        """
        collab = (
            mock.Mock(side_effect=roster)
            if isinstance(roster, BaseException)
            else mock.Mock(return_value=roster)
        )
        with mock.patch.object(azure_client, "list_repo_collaborators", collab):
            with mock.patch.object(
                azure_client, "_current_identity", return_value={"login": login}
            ):
                return azure_client.get_repo_permissions(
                    self.OWNER, "widget-service", host="dev.azure.com"
                )

    def test_a_confirmed_member_gets_write(self):
        perms = self._perms(roster=[{"login": "ada", "role_name": "member"}])
        self.assertTrue(perms["push"])
        self.assertTrue(perms["triage"])

    def test_a_non_member_is_denied_write(self):
        perms = self._perms(roster=[{"login": "someone-else", "role_name": "member"}])
        self.assertFalse(perms["push"])
        self.assertFalse(perms["triage"])

    def test_an_unreadable_roster_is_denied_write(self):
        """The fail-closed case: the lookup errored, so membership is unknown."""
        perms = self._perms(roster=ProviderCliError("teams unreadable"))
        self.assertFalse(perms["push"], "unknown membership must not grant push")
        self.assertFalse(perms["triage"], "unknown membership must not grant triage")

    def test_read_access_survives_an_unreadable_roster(self):
        # Reading is already demonstrated by the project being reachable, so
        # failing closed on WRITE must not also blind the list views.
        self.assertTrue(self._perms(roster=ProviderCliError("teams unreadable"))["pull"])

    def test_a_missing_identity_is_denied_write(self):
        # No login to compare means membership cannot be established either.
        perms = self._perms(roster=[{"login": "ada"}], login="")
        self.assertFalse(perms["push"])
