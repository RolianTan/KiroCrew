...
def _executable(directory: str, name: str = "az") -> str:
    ...
    with open(path, "w", encoding="utf-8") as handle:
        handle.write("#!/bin/sh\nexit 0\n")
    # 0o700, not 0o755: the resolver only needs `os.access(path, X_OK)` to hold for
    # THIS user, so group and other bits buy nothing. Keeping them off also matches
    # what the code under test wants of a real binary, since it refuses one that is
