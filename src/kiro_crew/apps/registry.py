...
async def _unpoison_rejected_checkout(
    ...
    *subdirectory*, when set, is the untrusted registry-declared subdirectory
    that *manifest_relpath* was built from (``f"{subdirectory}/app.json"``).
    A build step or ``onInstall`` script runs with write access to the
    checkout BEFORE some callers reach this cleanup, and can replace the
    subdirectory with a symlink after an earlier containment check already
    passed — this restore then runs unsandboxed as the Kiro Crew process, so
    it must not trust that earlier check. Containment is re-verified HERE,
    ...
    if subdirectory and _contained_join(pkg_dir, subdirectory) is None:
        # subdirectory no longer resolves inside pkg_dir RIGHT NOW — some
        # callers reach this point after a build step or onInstall script ran
        # with write access to the checkout, so a containment check the
        # caller made earlier cannot be trusted here. Both the raw write and
        # the git-checkout fallback below join manifest_relpath (built from
        # this same subdirectory) onto pkg_dir, so either would follow a
        # symlink planted in the meantime and write outside pkg_dir as this
        # unsandboxed process. Skip the manifest restore entirely rather than
        # risk that write; the rollback above already ran and stands.
        log_lines.append(
            f"WARNING: subdirectory {subdirectory!r} no longer resolves inside "
...
def _report_retained_stale_checkouts(
    build_result: dict[str, Any] | None, log_lines: list[str]
) -> None:
    """Log a "Previous checkout retained at" line for each moved-aside
    checkout that will actually stay retained on this exit.

    ``_pending_stale_cleanup`` collects every move-aside regardless of
    reason, but ``_restorable_stale`` (a subset) is put back by the
    enclosing ``finally`` — after this function's caller has already
    returned and its ``log_lines`` were joined into the response. Reporting
    a restorable path here would tell the user their checkout sits at a
    `.stale-*` directory that the `finally` is about to delete by restoring
    it: the exact misleading-recovery report this guards against. Only
    entries NOT in ``_restorable_stale`` are genuinely stranded, so only
    those are reported.
    """
    if build_result is None:
        return
    restorable = set(build_result.get("_restorable_stale") or [])
    ...
    try:
        ...
        if install_script:
            ...
            try:
                ...
                _report_retained_stale_checkouts(build_result, log_lines)
                outcome = {
                    "ok": False,
                    "name": name,
