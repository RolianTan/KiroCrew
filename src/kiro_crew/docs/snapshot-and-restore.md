...
You can opt out. Write `{"redact_uploads": false}` to `redaction.json` inside your backup
directory and the complete bundle is uploaded instead. That makes the off-host copy restore
a working install, and it puts usable credentials in the bucket — a deliberate trade, not a
default. If redaction is on and cannot be completed, the upload is refused; it never falls
back to sending the unredacted bundle.

The switch is a FILE there rather than a setting in `config.json`, and that placement is
the point. `config.json` is readable and writable by the agent, so a switch living there
could be flipped by the agent itself — and flipping this one publishes your live
credentials into object storage through a code path that looks entirely legitimate. The
backup directory is already fenced for the same reason its destination record is: neither
the agent's file tools nor any shell form can read or write it. Only you can. A missing
file means redact, and so does a damaged one, so the protective answer needs no file and a
corrupted switch cannot downgrade it.
