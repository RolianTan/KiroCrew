...
_DOTTED_BEARER = re.compile(
    r"\b[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{6,}\.[A-Za-z0-9_-]{25,}\b"
)
...
def redaction_switch_path() -> "Path":
    """Where the outbound-redaction opt-out lives: inside the keystone backup directory.
    ...
    security ceilings (the deny list, the computer-use enable) out of it. Turning outbound
    redaction off makes the next upload publish live credentials into object storage, so it
    is a ceiling and not a preference: a switch the agent can reach is one it can use to
    arrange its own exfiltration through a sanctioned path. The backup directory is already
    fenced for exactly that reason on account of the DESTINATION record -- one control on
    this path was protected and this one was not.
    ...
    Absent means redacted and unreadable means redacted, so the protective answer needs no
    file and a damaged one cannot downgrade it. The operator writes it out of band; no
    agent tool or shell form can.
    """
    return config_dir() / "backup" / "redaction.json"


def outbound_redaction_enabled() -> bool:
    """True unless the operator has explicitly opted out; fail-closed on anything else."""
    ...
    if not isinstance(raw, dict):
        # A list or a bare scalar is a damaged switch, not an opt-out. Reading it as one
        # would let a corrupted file turn redaction OFF, which is the direction that must
        # never happen by accident.
        return True
    # Only the exact boolean turns it off: a string "false" or a 0 is a config mistake, and
    # resolving a mistake towards publishing credentials is not a trade to make silently.
    return raw.get("redact_uploads") is not False
