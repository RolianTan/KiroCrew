...
import { checkChunkBudgets, formatBytes } from './lib/bundleReport.mjs'
...
export const CHUNK_BUDGETS = {
  ...
  // Monaco's editor core -- the code-editor engine, already lazy-loaded and
  // not splittable further (a monolithic upstream artifact).
  'editor.api2': 3720 * KB, // measured 3540 KB
...
function loadSummary(file) {
  ...
  if (parsed.version !== 1) {
    ...
    fail(`${file} has version ${JSON.stringify(parsed.version)}; this gate understands 1.`, 3)
  }
  return parsed
}
