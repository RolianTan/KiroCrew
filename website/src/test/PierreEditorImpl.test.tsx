...
describe('PierreEditorImpl imperative handle', () => {
  ...
  it('clamps a non-positive line onto the first row', () => {
    ...
    const lines = editor
      .of('setSelections')
      .map(c => (c.args[0] as { start: { line: number } }[])[0].start.line)
    expect(lines).toEqual([0, 0])
  })

  it('scrolls further down for a later line and never above the top', () => {
    ...
    const farDown = scroller.scrollTop

    expect(atTop).toBe(0)
    expect(midway).toBeGreaterThan(atTop)
    expect(farDown).toBeGreaterThan(midway)
  })

  it('keeps a near-top target pinned to the top on a tall viewport', () => {
    // Centring the target would ask for a negative offset here; the floor is
    // what keeps the row on screen instead of clamping to a blank scroll top.
    const { view, ref } = mount()
    attachEditor()
    const scroller = scrollerOf(view)
    Object.defineProperty(scroller, 'clientHeight', { value: 900, configurable: true })

    act(() => ref.current!.jumpToLine(4))

    expect(scroller.scrollTop).toBe(0)
  })

  it('jumps and focuses harmlessly before the editor attaches', () => {
    // The panel can wire the handle and call it from a route effect before
    // Pierre's deferred attach lands.
    const { view, ref } = mount()
    const scroller = scrollerOf(view)

    expect(() => {
      act(() => ref.current!.jumpToLine(90))
