...
describe('FilesHomePanel header', () => {
  ...
  it('invalidates both project queries from the refresh button', async () => {
    ...
    const { qc } = mount()
    // This control exists only while the rail is absent, so let the endpoint
    // fail first — on a healthy tree the rail owns refresh instead.
    await waitFor(() => expect(within(header()).getByLabelText('Refresh')).toBeInTheDocument())
    const spy = vi.spyOn(qc, 'invalidateQueries')
    fireEvent.click(within(header()).getByLabelText('Refresh'))
...
describe('FilesHomePanel tree availability', () => {
  ...
  it('withdraws the rail and explains itself once the tree endpoint errors', async () => {
    H.api.projectTree.mockRejectedValue(new Error('not a directory'))
    mount()
