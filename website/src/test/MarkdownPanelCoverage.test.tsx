...
describe('MarkdownPanel — save and cancel', () => {
  ...
  /**
   * The browser rail re-targets the tab it is docked in, which destroys the
   * buffer without going through any close path. `requestNavigate` is how that
   * gesture asks first; SidePanel's rail hands it every tree click.
   */
  it('blocks a guarded navigation while the buffer is dirty', () => {
    vi.spyOn(window, 'confirm').mockReturnValue(false)
  ...
  it('navigates a clean buffer without asking', () => {
    vi.spyOn(window, 'confirm').mockReturnValue(false)
    const ref = createRef<MarkdownPanelHandle>()
