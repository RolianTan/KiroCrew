...
import { File, FileDiff, MultiFileDiff, WorkerPoolContext } from '@pierre/diffs/react'
...
function PierreShell({ children }: { children: React.ReactNode }) {
  return <WorkerPoolContext.Provider value={workerPool}>{children}</WorkerPoolContext.Provider>
}

export function PierreCodeImpl({ file, options, className, langHint }: {
  file: FileContents
  ...
  return (
    <PierreShell>
      <File className={className} file={resolvedFile} options={resolved} />
    </PierreShell>
  )
}
