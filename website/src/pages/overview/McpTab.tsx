...
import { RefreshCw, Plug, AlertTriangle, Check, ChevronRight, Zap, X, Download, Braces, KeyRound } from 'lucide-react'
...
function mcpSignInRequired(s: McpServer): boolean {
  return s.status === 'needs_auth' && !!s.authScheme && s.authGrantPresent === false
}
...
function mcpStatusHint(status: string, serverName: string, signInRequired = false): string | undefined {
  ...
  if (signInRequired) return i18nT('pages.overview.mcpTab.sign_in_required_help')
...
export default function McpTab({ onManagedProviderClick }: McpTabProps = {}) {
  ...
  /**
   * Authorize = open a fresh chat session.
   *
   * The consent URL is not the dashboard's to mint: kiro-cli runs the OAuth
   * exchange and emits `_kiro.dev/mcp/oauth_request` while it brings MCP servers
   * up, which the chat surface renders as an approval banner. A NEW session is
   * what forces that bring-up — an already-running one has finished its init and
   * will not re-offer the prompt — so this button opens one and hands the user
   * to the only surface where authorization can actually complete.
   */
                ...
                <td className="px-2.5 py-2 border-b border-border text-[13px] w-full">
                  {s.status === 'error' && s.error ? (
                    <span className="text-danger text-[12px]">
                      ...
                      {!!s.authScheme && !!s.headers && (
                        <span className="block text-muted mt-0.5">{i18nT('pages.overview.mcpTab.oauth_not_a_static_token')}</span>
                      )}
                    </span>
                  ) : s.tools?.length ? (<div>
                    ...
                    <div className="flex gap-1 justify-end">
                      {mcpSignInRequired(s) && (
                        <Btn
                          ...
                          title={i18nT('pages.overview.mcpTab.authorize_help', { name: s.name })}
                        ><KeyRound size={13} /> {i18nT('pages.overview.mcpTab.authorize')}</Btn>
                      )}
                      {s.kirocrewManaged && (
