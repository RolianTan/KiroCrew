...
interface SidePanelProps {
  ...
  slot: string
  onFileOpen?: (path: string, opts?: { replaceId?: string; line?: number; endLine?: number; diffMode?: boolean }) => void
...
function FileTabBody({ tab, projectDir, onContentChange, onDiffModeChange, onFileSave, onFileOpen, onClose, onSubmitComments, onRevealConsumed }: {
  ...
  onFileOpen?: (p: string, opts?: { diffMode?: boolean; replaceId?: string }) => void
  onClose: () => void
    ...
    <MarkdownPanel
      ...
      browserRail={railUsable ? (
        <FileBrowserRail
          ...
          onFileOpen={(abs, diff) => {
            const nav = () => onFileOpen(abs, { diffMode: diff, replaceId: tab.id })
