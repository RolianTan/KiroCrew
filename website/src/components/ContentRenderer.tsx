...
export const ContentRenderer = memo(function ContentRenderer({
  ...
  const inner = (
    <>
      ...
      {!isRichType && !editing && !isMarkdown && (
        <div className={`relative w-full h-full overflow-hidden ${flush ? '' : 'border border-border rounded-md'}`}>
          {/* previewRef lands on the code host (not just the markdown branch)
              so a selection in a text/json body still has a root to map back
              to source for the comment flow. */}
          <div
            ref={previewRef as React.RefObject<HTMLDivElement>}
            className="absolute inset-0 overflow-auto pierre-surface"
          >
            <PierreCode file={codeFile} langHint={lang} options={codeOptions} />
          </div>
        </div>
      )}
    </>
